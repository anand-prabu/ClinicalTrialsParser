...
def get_status(jsonFile):
    ...
    return json_data['clinical_study']['overall_status']
...
def get_phase(jsonFile):
    json_file = open(jsonFile).read()
    json_data = json.loads(json_file)
    return json_data['clinical_study']['phase']
...
"""Gets list of all json files in specified directory"""
list_of_json=[]
path="ct_raw_json/"
dirList=os.listdir(path)
for fname in dirList:
    list_of_json.append(path+fname)

"""Prints the status statistics of json files"""
print bulk_get_status(list_of_json)
